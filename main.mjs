...
const rawValidations = readFileSync(new URL('./validations.json', import.meta.url), 'utf-8');
const validations = JSON.parse(rawValidations).filter(x=>x.enabled).map(x=>Object.assign(x, {hash_id:hashIdGen(x)}));

const rawMailRecipients = readFileSync(new URL('./mailRecipients.json', import.meta.url), 'utf-8');
    ...
    const request = mailjet
        ...
        .request({
          Messages: [
            {
              From: {
                Email: "sender@zerodaybootcamp.xyz",
                Name: "haHishook Agent"
              },
              To: mailRecipients,
...
const scanProducts = async function(){
  ...
  try {
    refactoryPeriods = JSON.parse(readFileSync(new URL('./refactoryPeriods.json', import.meta.url), 'utf-8'));
  } catch (err) {
    ...
    if(isSuccess){
      if(!(refactoryPeriods[hashIdGen(validation)]) || (refactoryPeriods[hashIdGen(validation)]<current_timestamp)){
        ...
        if(!(refactoryPeriods[hashIdGen(validation)])){
          ...
          writeFileSync('./refactoryPeriods.json', JSON.stringify(refactoryPeriods, null, 2))
        }
      }else{

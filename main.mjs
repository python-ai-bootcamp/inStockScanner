...
async function main() {
    logger("Starting main execution");
    let browser;
    try {
        ...
        for (const providerConfig of notificationProviders.filter(x => x.enabled)) {
            try {
                ...
                logger({
                    message: `!!! CRITICAL ERROR in initializing for ${providerConfig.provider} provider !!!`,
                    error
                });
            }
        }
        ...
        let results;
        try {
            results = await scanProducts(browser);
        } catch (error) {
            logger({ message: "!!! CRITICAL ERROR in scanProducts !!!", error });
            results = []; // Return an empty array on error
        }
        ...
        if (results.length > 0) {
            ...
            for (const providerConfig of notificationProviders.filter(x => x.enabled)) {
                ...
                try {
                    ...
                    logger({
                        message: `!!! CRITICAL ERROR in sending mail for ${providerConfig.provider} provider !!!`,
                        error
                    });
                }
            }
        ...
        if (browser) {
            await browser.close();
            logger("Browser closed in main finally block.");
        }
    }
}
...
main().catch(error => {
    logger({
        message: "!!! TOP LEVEL CRITICAL ERROR in main function !!!",
        error
    });
});

    ...
    while(!connected) {
        try {
            const connectionPromise = new Promise(async (resolve, reject) => {
                ...
                const authFile = path.join('./configuration', 'baileysSessions', `baileys-${clientId}.json`);
                ...
                const connectionUpdateListener = (update) => {
                    ...
                    if (connection === 'open') {
                        sock.ev.off('connection.update', connectionUpdateListener);
...
export async function sendNotification({ logger, recipients, textContent }) {
  ...
  for (const recipient of recipients) {
    try {
      const jid = `${recipient.phoneNumber.replace(/\D/g, '')}@c.us`;
